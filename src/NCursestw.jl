...
@BinDeps.load_dependencies [:ncursestw => :ncursestw]
...
function get_nc_version()
    nc_ver = ccall( dlsym( ncursestw, :curses_version ), Ptr{UInt8}, ())
    if nc_ver != C_NULL
...
function initscr()
    ccall( dlsym( ncursestw, :initscr), Ptr{Void}, () )
end

function endwin()
    ccall( dlsym( ncursestw, :endwin), Int, () )
end

function isendwin()
    ccall( dlsym( ncursestw, :isendwin), Bool, () )
end

function newwin( lines::Int, cols::Int, origy::Int, origx::Int )
    ccall( dlsym( ncursestw, :newwin ), Ptr{Void}, ( Int, Int, Int, Int ),
        lines, cols, origy, origx )
end

function subwin( win::Ptr{Void}, lines::Int, cols::Int, origy::Int, origx::Int )
    ccall( dlsym( ncursestw, :subwin ), Ptr{Void}, ( Ptr{Void}, Int, Int, Int, Int ),
        win, lines, cols, origy, origx )
end

function derwin( win, lines::Int, cols::Int, origy::Int, origx::Int )
    ccall( dlsym( ncursestw, :derwin ), Ptr{Void}, ( Ptr{Void}, Int, Int, Int, Int ),
        win, lines, cols, origy, origx )
end

function delwin( win::Ptr{Void} )
    ccall( dlsym( ncursestw, :delwin ), Void, ( Ptr{Void}, ), win )
end

function mvwaddch( win, y::Int, x::Int, c )
    ccall( dlsym( ncursestw, :mvwaddch), Void,
        ( Ptr{Void}, Int, Int, Int ), win, y, x, c )
end

function mvwprintw( win::Ptr{Void}, row::Int, height::Int, fmt::String, str::String )
    ccall( dlsym( ncursestw, :mvwprintw), Void,
        ( Ptr{Void}, Int, Int, Ptr{Uint8}, Ptr{Uint8}),
        win, row, height, fmt, str )
end

function printw( str::String )
    ccall( dlsym( ncursestw, :printw), Void, (Ptr{Uint8},), str )
end

function mvprintw( row::Int, height::Int, str::String )
    ccall( dlsym( ncursestw, :printw), Void, (Int, Int, Ptr{Uint8}), row, height, str )
end

function wprintw( win::Ptr{Void} , str::String )
    ccall( dlsym( ncursestw, :printw), Void, (Ptr{Void}, Ptr{Uint8}), win, str )
end

function wmove( win, y::Int, x::Int )
    ccall( dlsym( ncursestw, :wmove), Int, ( Ptr{Void}, Int, Int ), win, y, x )
end

function wrefresh( win::Ptr{Void} )
    ccall( dlsym( ncursestw, :wrefresh ), Void, ( Ptr{Void}, ), win )
end

function touchwin( win::Ptr{Void} )
    ccall( dlsym( ncursestw, :touchwin ), Void, ( Ptr{Void}, ), win )
end

function refresh()
    ccall( dlsym( ncursestw, :refresh ), Void, ( ) )
end

function erase()
    ccall( dlsym( ncursestw, :erase ), Void, () )
end

function werase( win::Ptr{Void} )
    ccall( dlsym( ncursestw, :werase ), Void, (Ptr{Void},), win )
end

function wclear( win::Ptr{Void} )
    ccall( dlsym( ncursestw, :wclear ), Void, (Ptr{Void},), win )
end

function box( win, vchr, hchr )
    ccall( dlsym( ncursestw, :box ), Void, (Ptr{Void}, Char, Char), win, vchr, hchr )
end

function wgetch( win::Ptr{Void } )
    ccall( dlsym( ncursestw, :wgetch ), Uint32, (Ptr{Void},), win )
end

function keypad( win, bf )
    ccall( dlsym( ncursestw, :keypad ), Int, (Ptr{Void}, Bool), win, bf )
end

function cbreak()
    ccall( dlsym( ncursestw, :cbreak), Void, ( ) )
end

function nocbreak()
    ccall( dlsym( ncursestw, :nocbreak), Void, ( ) )
end

function echo()
    ccall( dlsym( ncursestw, :echo), Void, ( ) )
end

function noecho()
    ccall( dlsym( ncursestw, :noecho), Void, ( ) )
end

function nodelay( win, bf )
    ccall( dlsym( ncursestw, :nodelay ), Int, (Ptr{Void}, Bool), win, bf )
end

function raw()
    ccall( dlsym( ncursestw, :raw ), Int, () )
end

function noraw()
    ccall( dlsym( ncursestw, :noraw ), Int, () )
end

function notimeout( win, bf )
    ccall( dlsym( ncursestw, :notimeout ), Int, (Ptr{Void}, Bool), win, bf )
end

function timeout( delay::Int )
    ccall( dlsym( ncursestw, :timeout ), Void, (Int,), delay )
end

function wtimeout( win, delay::Int )
    ccall( dlsym( ncursestw, :wtimeout ), Void, (Ptr{Void}, Int), win, delay )
end

# not standard but convenient
function getwinmaxyx( win )
    maxy = ccall( dlsym(ncursestw, :getmaxy), Int, ( Ptr{Void}, ), win )
    maxx = ccall( dlsym(ncursestw, :getmaxx), Int, ( Ptr{Void}, ), win )
    ( maxy, maxx )
end

function getwinbegyx( win )
    maxy = ccall( dlsym(ncursestw, :getbegy), Int, ( Ptr{Void}, ), win )
    maxx = ccall( dlsym(ncursestw, :getbegx), Int, ( Ptr{Void}, ), win )
    ( maxy, maxx )
end

function mvwin( win, y::Int, x::Int )
    ccall( dlsym( ncursestw, :mvwin), Int, ( Ptr{Void}, Int, Int ), win, y, x )
end

function beep()
    ccall( dlsym( ncursestw, :beep ), Void, () )
end

function flash()
    ccall( dlsym( ncursestw, :flash), Void, () )
end

function is_term_resized( lines::Int, cols::Int )
    ccall( dlsym( ncursestw, :is_term_resized ), Bool, (Int, Int), lines, cols )
end

function wresize( win, lines::Int, cols::Int )
    ccall( dlsym( ncursestw, :wresize), Int, (Ptr{Void}, Int, Int), win, lines, cols )
end

function getcuryx( win )
    cury = ccall( dlsym(ncursestw, :getcury), Cint, ( Ptr{Void}, ), win )
    curx = ccall( dlsym(ncursestw, :getcurx), Cint, ( Ptr{Void}, ), win )
    ( cury, curx )
end

function start_color()
    ccall( dlsym( ncursestw, :start_color), Void, () )
end

function init_pair( pair, f, b )
    ccall( dlsym( ncursestw, :init_pair ), Int, ( Int16, Int16, Int16 ), pair, f, b )
end

function init_color( color, r,g,b )
    ccall( dlsym( ncursestw, :init_color ), Int, ( Int16, Int16, Int16, Int16 ),
        color, r,g,b )
end

function has_colors()
    ccall( dlsym( ncursestw, :has_colors), Bool, () )
end

function wattroff( win, attrs )
    ccall( dlsym(ncursestw, :wattroff), Int, ( Ptr{Void}, Uint32 ), win, attrs )
end

function wattron( win, attrs )
    ccall( dlsym(ncursestw, :wattron), Int, ( Ptr{Void}, Uint32 ), win, attrs )
end

function wattrset( win, attrs )
    ccall( dlsym(ncursestw, :wattrset), Int, ( Ptr{Void}, Uint32 ), win, attrs )
end

function wbkgdset( win, ch )
    ccall( dlsym(ncursestw, :wbkgdset ), Void, ( Ptr{Void}, Uint32 ), win, ch )
end

function wbkgd( win, ch )
    ccall( dlsym(ncursestw, :wbkgd ), Void, ( Ptr{Void}, Uint32 ), win, ch )
end

function curs_set( vis )
    ccall( dlsym(ncursestw, :curs_set), Int, ( Int, ), vis )
end

function has_mouse()
    ccall( dlsym(ncursestw, :has_mouse), Bool, () )
end

function mousemask( mask )
    oldmm = Array( Uint64, 1 )
    resultmm = ccall( dlsym( ncursestw, :mousemask), Uint64, (Uint64, Ptr{Uint64}), mask, oldmm )
    ( resultmm, oldmm[1])
end

function mouseinterval( n::Int )
    ccall( dlsym(ncursestw, :mouseinterval), Int, (Int, ) , n)
end
...
function getmouse()
    ...
    ccall( dlsym( ncursestw, :getmouse), Int, (Ptr{Uint8}, ), mouseByteString )
    bs = mouseByteString
    x = uint8(bs[5])
    y = uint8(bs[9])
    state=:unknown
...
function baudrate()
    ccall( dlsym( ncursestw, :baudrate), Int, () )
end

function clearok( win, bf )
    ccall( dlsym( ncursestw, :clearok), Int, ( Ptr{Void}, Bool ), win, bf )
end

function immedok( win, bf )
    ccall( dlsym( ncursestw, :immedok), Int, ( Ptr{Void}, Bool ), win, bf )
end

function napms( ms::Int )
    ccall( dlsym( ncursestw, :napms), Int, (Int, ), ms )
end
